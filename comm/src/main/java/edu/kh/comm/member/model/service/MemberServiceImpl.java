...
@Service // 비즈니스로직(데이터 가공, DB연결)을 처리하는 클래스임을 명시 + bean등
public class MemberServiceImpl implements MemberService{
	 ...
	 *    어쩔수없이 Connection을 Service에서 얻어올수밖에 없었다.
	 * */
	...
	public Member login(Member inputMember) {
		...
		// 전달받은 비밀번호를 암호화하여 DB에서 조회한 비밀번호와 비교 (DB에서 비교 X)
		...
		// B 회원 / 비밀번호 1234 -> 암호화 : @bdg
		
		// * 매번 암호화되는 비밀번호가 달라져서 DB에서 직접 비교 불가능
		// 대신 Bcrypt 암호화를 지원하는 객체가 이를 비교하는 기능(메서드)을 가지고 있어서 이를 활용하면 됨.
		
		// ** Bcrypt 암호화를 사용하기 위해 이를 지원하는 Spring-security 모듈 추가 **
		// maven에서 spring-security 검색 후 web, config, core 위에서 3개 5.7.1버전 pom.xml에 추가하기
		logger.debug(inputMember.getMemberPw() + " / "  + bcrypt.encode(inputMember.getMemberPw()));
//		logger.debug(inputMember.getMemberPw() + " / "  + bcrypt.encode(inputMember.getMemberPw()));
//		logger.debug(inputMember.getMemberPw() + " / "  + bcrypt.encode(inputMember.getMemberPw()));
//		logger.debug(inputMember.getMemberPw() + " / "  + bcrypt.encode(inputMember.getMemberPw()));
//		logger.debug(inputMember.getMemberPw() + " / "  + bcrypt.encode(inputMember.getMemberPw()));  비크립트 암호화 확인용도
		...
		if(loginMember != null) { // 일치하는 이메일을 가진 회원 정보가 있을 경우
			...
			if(bcrypt.matches(inputMember.getMemberPw(), loginMember.getMemberPw() )) { // 비밀번호가 일치할 경우
				...
				loginMember.setMemberPw(null); // 로그인멤버가 세션에 올라가는데 비번은 세션에 올라가면 안됨, 비교 끝난 후 비밀번호 지우기!
		...
		// Connection을 얻어오거나/반환하거나 트랜잭션 처리를 하는 구문을 작성하지 않아도
