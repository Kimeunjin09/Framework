...
public class MyPageServiceImpl implements MyPageService {
	...
	public int secession(Member loginMember) {
		...
		// 1) DB에서 암호화된 비밀번호를 조회하여 입력받은 비밀번호와 비교 (위에 만들어둔 selectEncPw쓸거)
	...
	public int updateProfile(Map<String, Object> map) throws IOException{
		...
		if(delete.equals("0")) { // 이미지가 변경된 경우
			...
			// 2023042294211234.png 이런식으로 이름이 들어오게됨
			
			map.put("profileImage", map.get("webPath") + renameImage);
			//		/resources/images/memberProfile/2023042294211234.png
		...
		if(result > 0 && map.get("profileImage") != null) {
			// uploadImage.transferTo(new File(서버경로 + 파일명)); // 이미지를 내가 원하는 곳으로 옮김
